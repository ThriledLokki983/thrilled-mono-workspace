...
class PasswordManager {
    ...
    async hashPassword(password) {
        try {
            ...
            const salt = await bcrypt_1.default.genSalt(this.config.saltRounds);
    ...
    calculatePasswordStrength(password) {
        ...
        const hasCommonPattern = commonPatterns.some((pattern) => password.toLowerCase().includes(pattern));
        if (!hasCommonPattern)
            score += 10;
        else
     ...
     */
    async createResetToken(userId, expiresInMinutes = 30) {
        try {
            ...
            await this.cache.setObject(`${this.resetTokenPrefix}${hashedToken}`, resetData, expiresInMinutes * 60);
            // Store user->token mapping for cleanup
            await this.cache.set(`${this.resetTokenPrefix}user:${userId}`, hashedToken, expiresInMinutes * 60);
    ...
    async verifyResetToken(token) {
        try {
            const hashedToken = (0, crypto_1.createHash)('sha256').update(token).digest('hex');
            const resetData = await this.cache.getObject(`${this.resetTokenPrefix}${hashedToken}`);
            if (!resetData) {
            ...
            // Mark token as used
            resetData.used = true;
            await this.cache.setObject(`${this.resetTokenPrefix}${hashedToken}`, resetData, 60 // Keep for 1 minute to prevent reuse
            );
     ...
     */
    async revokeResetTokens(userId) {
        try {
     ...
     */
    async trackResetAttempt(identifier) {
        try {
     ...
     */
    updatePasswordPolicy(newConfig) {
